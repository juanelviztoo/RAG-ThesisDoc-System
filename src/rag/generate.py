...
# =========================
# Per-method answer + per-method bukti policy
# =========================
...
_METHOD_MATCH_PATTERNS = {
    "RAD": [r"\brad\b", r"rapid application development"],
    "PROTOTYPING": [r"\bprototyping\b", r"\bmetode\s+prototyp", r"\bmodel\s+prototyp"],
...
def _has_steps_signal_local(text: str) -> bool:
    """
    Sinyal kuat bahwa chunk memuat tahapan (mirip seperti has_steps_signal di app_streamlit,
    namun versi lokal agar generate.py mandiri).
    """
    t = _strip_doc_header_line(text or "").lower()
    if not t:
        return False
...
def _ensure_all_methods_mentioned(out: str, *, target_methods: List[str]) -> bool:
    """
    diatur lebih strict — semua metode harus punya header line "<Label>:"
    """
    return _all_method_headers_present(out, target_methods)
